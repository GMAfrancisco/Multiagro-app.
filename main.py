...
# Esto evita que la aplicación se reinicie o pida login al subir fotos o cambiar tabs
...
# --- CSS DE ALTA VISIBILIDAD (CONTRASTE Y PROPORCIONES) ---
st.markdown(f"""
    <style>
    ...
    /* Cargador de archivos (Instrucciones en NEGRO para visibilidad) */
    ...
    /* Pestañas (Galería/Cámara) en BLANCO puro */
    ...
    /* Botón verde Cotizar/Iniciar (WhatsApp Style) */
    ...
    /* Footer de Marcas con fondo blanco para logos */
...
if img_final and st.button("🚀 INICIAR ASESORÍA"):
    with st.spinner("Analizando con jerarquía fitosanitaria..."):
        try:
            genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
            model = genai.GenerativeModel('gemini-2.0-flash-lite')
            
            # PROMPT CON JERARQUÍA DE PRIORIDAD (Plaga -> Hongo/Bacteria -> Nutrición)
            prompt = f"""
            RESPONDE 100% EN ESPAÑOL. Eres el Agrónomo Senior de Grupo Multiagro. 
            Tu misión es identificar el problema en {cultivo_input} con el siguiente descarte:
            1. PRIORIDAD ALTA: Busca insectos, ácaros, larvas o rastros de masticación/raspado (Plagas).
            2. PRIORIDAD MEDIA: Busca manchas, micelio, pudrición o deformaciones (Hongos, Bacterias, Virus).
            3. PRIORIDAD BAJA: Si no hay daño por plagas o patógenos, analiza clorosis o quemaduras (Deficiencias Nutricionales).
...
# 5. REGISTRO DE PRODUCTOR (PROVINCIAS RD)
st.divider()
...
with st.form("registro_p"):
    ...
    if st.form_submit_button("✅ ENVIAR MIS DATOS"):
        if nom and tel:
            if registrar_en_odoo(nom, ema, tel, prov): st.success("¡Datos registrados con éxito!")
