...
st.markdown("""
    ...
    .product-card { background-color: #1E1E26; border-radius: 25px; padding: 25px; border: 1px solid #3E3E4A; text-align: center; margin-bottom: 20px; display: flex; flex-direction: column; justify-content: space-between; height: 100%; position: relative; transition: transform 0.3s ease; color: #FFFFFF !important; }
    .product-card:hover { transform: translateY(-5px); } 
    .product-card h4 { color: #FFFFFF !important; font-size: 0.9rem; margin-top: 10px; }
    ...
    .logo-container { display: flex; justify-content: center; align-items: center; height: 100px; background: #FFFFFF; border-radius: 20px; padding: 15px; margin-bottom: 15px; border: 1px solid #DDD;}
...
def get_odoo_prods():
    try:
        ...
        if uid:
            ...
            def es_favorito(p): return 1 if str(p.get('priority', '0')) == '1' else 0
...
def registrar_cliente_odoo(nombre, email, telefono, lugar):
    try:
        ...
        if uid:
            ...
            return models.execute_kw(db, uid, key, 'res.partner', 'create', [{'name': nombre, 'email': email, 'phone': telefono, 'comment': f'App AgTech Multiagro | Prov: {lugar}'}])
    except: return None
...
if not st.session_state.authenticated:
            ...
            if not puede_consultar(st.session_state.user_email, st.session_state.user_tier):
                ...
                with st.spinner("Analizando..."):
                    try:
                        ...
                        prompt = f"RESPONDE 100% EN ESPAÑOL. Eres Ing. Agrónomo y Fitopatólogo de Multiagro. Analiza la planta: {cultivo_input if cultivo_input else 'No especificado'}. 1. DIAGNÓSTICO: Identifica la plaga, hongo, bacteria o deficiencia nutricional. 2. RECETA EXACTA: ESTÁS OBLIGADO a recomendar EXACTAMENTE 4 productos diferentes de la lista de abajo. REGLA VITAL: Lee atentamente los 'Ingredientes/Notas' de cada producto en el inventario. Usa tu conocimiento científico para saber qué ingrediente activo de nuestra lista erradica el problema diagnosticado. Fíjate que la categoría entre corchetes coincida. \n\n{inventario_ia}\n\nEscribe el Nombre Comercial de los 4 productos recomendados en NEGRITAS. 3. APLICACIÓN: Explica brevemente cómo el ingrediente activo actúa sobre el problema y da labores culturales."
                        ...
                        st.session_state.chat_history = [
                            {"role": "user", "parts": [f"Contexto oculto: Analizaste {cultivo_input}."]},
    ...
    if st.session_state.chat_history:
        ...
        # EL ENLACE APLICADO CON LA ESTRUCTURA DEL CÓDIGO VIEJO QUE FUNCIONA
        ...
        if st.session_state.prods_filtrados:
            ...
            for i, p in enumerate(st.session_state.prods_filtrados):
                with cols_ia[i % 2]:
                    ...
                    st.markdown(f"<div class='product-card'>{badge}{img_b64}<h4 style='font-size: 0.9rem; margin-bottom: 5px;'>{nombre_corto}</h4><p style='color: #007BFF; font-weight: bold;'>RD$ {p['list_price']:,.2f}</p></div>", unsafe_allow_html=True)
    ...
    if st.session_state.carrito:
        ...
        enlace_whatsapp_carrito = f"https://wa.me/18295624653?text=Hola Multiagro, mi correo es {st.session_state.user_email} y me interesa cotizar lo siguiente: {productos_unidos}"
        
        st.link_button("📲 Enviar Cotización a un Asesor", enlace_whatsapp_carrito, type="primary", use_container_width=True)
    ...
    st.markdown("<div style='text-align: center; color: #666666; font-size: 0.85rem; margin-top: 50px; padding-bottom: 20px;'>&copy; 2026 Grupo Multiagro.<br>Desarrollado con Inteligencia Artificial.</div>", unsafe_allow_html=True)
