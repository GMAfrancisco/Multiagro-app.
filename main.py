...
# Script de persistencia para evitar cierres de sesión por inactividad del navegador móvil
components.html("""
    <script>
    let tiempoOculto;
...
# =========================================================================
# 2. BLOQUE DE APARIENCIA (CSS COMPLETO Y DISEÑO MODERNO)
# =========================================================================
st.markdown("""
    <style>
    .stApp { 
...
# Listas de clasificación por palabras clave
...
def get_odoo_prods():
    try:
        ...
        if uid:
            ...
            # Límite amplio para asegurar que todo el catálogo cargue
...
if not st.session_state.authenticated:
    ...
    # Clasificación robusta de productos
    prods_medicina = []
    prods_semillas = []
    prods_riego = []
    prods_equipos = []
    ...
    # =========================================================================
    # LÓGICA DE INTELIGENCIA ARTIFICIAL (CON NOTAS INTERNAS)
    # =========================================================================
    if img is not None:
            ...
            if not puede_consultar(st.session_state.user_email, st.session_state.user_tier):
                ...
                with st.spinner("Analizando..."):
                    try:
                        ...
                        1. DIAGNÓSTICO: Identifica la plaga, hongo, bacteria o deficiencia nutricional con certeza.
                        2. RECETA EXACTA: ESTÁS OBLIGADO a recomendar EXACTAMENTE 4 productos diferentes de la lista de abajo. 
                        REGLA VITAL: Lee atentamente los "Ingredientes/Notas" de cada producto en el inventario. Usa tu conocimiento científico para saber qué ingrediente activo de nuestra lista erradica el problema diagnosticado.
                        ...
                        4. SEGURIDAD: Advierte leer la etiqueta del fabricante.
                        """
                        ...
                        st.session_state.chat_history = [
                            {"role": "user", "parts": [f"Contexto oculto: Realizaste un análisis inicial de la planta {cultivo_input}."]},
    ...
    # =========================================================================
    # MOSTRAR EL CHAT Y BOTÓN WHATSAPP DEL TÉCNICO (MÉTODO CÓDIGO VIEJO)
    # =========================================================================
    if st.session_state.chat_history:
        ...
        # Usamos texto plano reemplazando espacios manualmente en vez de urllib para que Android no lo bloquee.
        texto_tecnico = "Hola, necesito consultar a un tecnico sobre un diagnostico en la app"
        texto_tecnico_formateado = texto_tecnico.replace(" ", "%20")
        enlace_tecnico = f"https://wa.me/18295624653?text={texto_tecnico_formateado}"
        
        st.link_button(
            label="👨‍🌾 Consultar dudas a un técnico por WhatsApp", 
            url=enlace_tecnico, 
            type="secondary", 
                ...
                with cols_ia[i % 2]:
                    ...
                    st.markdown(f"""
                        <div class="product-card">
                            {badge}
                            {img_b64}
                            <h4 style='font-size: 0.9rem; margin-bottom: 5px;'>{nombre_corto}</h4>
    ...
    # =========================================================================
    # CATÁLOGO E-COMMERCE (Pestañas completas)
    # =========================================================================
    st.divider()
    ...
    # =========================================================================
    # CARRITO DE WHATSAPP FLOTANTE (MÉTODO CÓDIGO VIEJO)
    # =========================================================================
    if st.session_state.carrito:
        st.markdown("<br><br><br>", unsafe_allow_html=True) 
        st.markdown('<div class="cart-box">', unsafe_allow_html=True)
        st.markdown(f"<h3>🛒 Tu Cotización ({len(st.session_state.carrito)} artículos)</h3>", unsafe_allow_html=True)
        
        # --- AQUÍ ESTÁ EL FIX DE WHATSAPP DEL CARRITO ---
        # Unimos la lista con guiones simples.
        productos_unidos = " - ".join(st.session_state.carrito)
        texto_carrito = f"Hola Multiagro, mi correo es {st.session_state.user_email} y deseo cotizar: {productos_unidos}"
        # Formateamos manualmente solo los espacios para evitar bloqueos del WebView
        texto_carrito_formateado = texto_carrito.replace(" ", "%20")
        enlace_carrito = f"https://wa.me/18295624653?text={texto_carrito_formateado}"
        
        st.link_button(
            label="📲 ENVIAR COTIZACIÓN A WHATSAPP", 
            url=enlace_carrito, 
            type="primary", 
            use_container_width=True
        )
